...
const MIN_LENGTH = 200; // 下限(明らかに手抜きな出力を弾く)
// 「程度」表現のレンジなので、上限は1割の超過まで許容してから再生成をかける
const LENGTH_SLACK = 1.1;
...
export class ClaudeProposalGenerator implements ProposalGenerator {
  ...
  async generate(
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      ...
      const issues = validateProposal(lastProposal, job, analysis?.recommendedLength);
...
- 適正分量: ${LENGTH_GUIDE[analysis.recommendedLength]} ※このレンジを厳守し、上限を超えないこと
 ...
 */
export const LENGTH_RANGES: Record<JobAnalysis['recommendedLength'], { min: number; max: number }> = {
  short: { min: 400, max: 600 },
  medium: { min: 600, max: 1000 },
  long: { min: 1000, max: 1600 },
};

const LENGTH_GUIDE: Record<JobAnalysis['recommendedLength'], string> = {
  short:
    '400〜600字(軽いタスク依頼に長文を送ると読まれない。即戦力の自信が伝わる鋭さで、要点を一点突破する)',
  medium:
    '600〜1000字(標準的な開発案件。具体性と読みやすさの両立。テンプレを埋めただけに見える中途半端な薄さに落とさない)',
  long: '1000〜1600字(要件が厚い案件。長さ自体が「依頼文を深く読み込んだ」というシグナルになる。ただし冗長な水増しは逆効果で、全文がこの案件固有の内容であること)',
};
...
# ルール
- 分量は案件分析の「適正分量」レンジを厳守する。上限超過は「読み込みの深さ」ではなく「編集力のなさ」のシグナルになるため、レンジを超えそうなら実績や自己PRを削って密度を上げる。逆に指定レンジ未満の中途半端な分量に落とすくらいなら、内容を深めてレンジに乗せる。1文字も無駄にしない
...
/**
 * 自己検査: 下限字数・適正分量の上限・案件キーワードの含有をチェックする。
 * 上限は案件分析のrecommendedLengthレンジ(LENGTH_RANGES)に基づき、
 * 「程度」表現を踏まえてLENGTH_SLACK分の超過まで許容する。
 * 分析が無い場合は最も寛容なlongの上限を適用する(調査根拠のレンジ外に膨らむのを防ぐ)。
 */
export function validateProposal(
  proposal: string,
  job: Job,
  recommendedLength?: JobAnalysis['recommendedLength'] | null,
): readonly string[] {
  const issues: string[] = [];
  if (proposal.length < MIN_LENGTH) issues.push(`${MIN_LENGTH}字未満(現在${proposal.length}字)`);

  const range = LENGTH_RANGES[recommendedLength ?? 'long'];
  const maxAllowed = Math.floor(range.max * LENGTH_SLACK);
  if (proposal.length > maxAllowed) {
    issues.push(
      `適正分量の上限${range.max}字を大きく超過(現在${proposal.length}字)。実績や自己PRを絞り、${range.min}〜${range.max}字に収めること`,
    );
  }
